...
// Arduino Mega (master)  - Arduino Mega (slave)    Uno 
// 53 (SS)              -- 53 (SS)                 -- 10         
// 50 (MISO)            -- 50 (MISO)               -- 12
// 51 (MOSI)            -- 51 (MOSI)               --11
// 52 (SCK)            --  52 (SCK)                -12
...
// ACC: A total num of LED = 186; each slave processes 40 LEDs
...
//ACC: const int NumPixels1 = 40;
//ACC: const int NumPixles2 = 44;
//ACC: const int NumPixels3 = 50;
//ACC: const int NumPixels4 = 52;
...
const int rightGroupsByteSize = NumPixelsRightGrouos * 3;
...
byte leftLED[ leftGroupsByteSize ];

byte rightLED1[rightGroup1ByteSize];
byte rightLED2[rightGroup2ByteSize];
byte rightLED3[rightGroup3ByteSize];
byte rightLED4[rightGroup4ByteSize];
byte rightLED5[rightGroup5ByteSize];
...
void setup (void) {
  ...
  pinMode(ssLeft, OUTPUT); // ssLeft: to the second mega master for the left side led array

 // to the 5 Unos for the right side LED array
  pinMode(ss1, OUTPUT);
  pinMode(ss2, OUTPUT);
  pinMode(ss3, OUTPUT);
  pinMode(ss4, OUTPUT);
 ...
 // prepare to send the bytes to the second master and the slave unos by SPI comm
