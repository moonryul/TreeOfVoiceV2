...
#define NUMPIXELS1 152 // Number of Pixies in the strip
...
const int m_totalBufferSize = (NUMPIXELS1 + 2) * 3; // 6 bytes are for the start and end bytes; But we use only the start byte in this code
...
void loop() {
  ...
  { // get the bytes between the startBytes and the endBytes
    ...
    for (int i = 1; i < NUMPIXELS1; i++) // i ==0: startByte; i = NUMPIXELS1: endByte
    {
